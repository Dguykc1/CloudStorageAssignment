...
import com.udacity.jwdnd.course1.cloudstorage.services.TheFileModelService;
...
@Controller
public class TheFileController {
    private UserService userFormService;
    private TheFileModelService fileModelService;

    public TheFileController(UserService userService, TheFileModelService fileModelService) {
        this.userFormService = userService;
        this.fileModelService = fileModelService;
    }

    private Logger logger = LoggerFactory.getLogger(TheFileController.class);
    ...
    public String uploadFile(@RequestParam("fileUpload") MultipartFile multipartFile, RedirectAttributes redirectAttributes,
        ...
        try {
            Integer userId= userFormService.getUser(authentication.getName()).getUserId();
            ...
            if((multipartFile.getSize() > 5242888)) {
            ...
            //manually create the theFileModel POJO with info from multipart file
            TheFileModel theFileModel =new TheFileModel(null,fileName,contentType,fileSize.toString(),userId,fb);
            fileModelService.insertFile(theFileModel);
    ...
    public String deleteFile(@PathVariable("filename") String fileName,Authentication authentication,RedirectAttributes redirectAttributes){
        ...
        logger.error("File delete controllerpackage");
        try{
